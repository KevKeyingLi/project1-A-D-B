...
def processFeedBack(Q, R, NR):
	...
	for word in sorted_dict:
		...
		if word[0] in Q:
			...
			if word_cnt < 2:
				#print 'Appending ' + word[0]
				qry_str += '%20' + word[0]
				QUERY += ' '+word[0]
				word_cnt += 1
	# print("Added query words"+qry_str)
	'''
...
if len(sys.argv) == 3:
	...
	while precision<TARGET_PRECISION:
		...
		round_cnt += 1
	if not fail:
		print("Succeeded after "+str(round_cnt)+(" rounds of feedback. \nFinal precision: %.4f" % precision)+".\n Final Query: "+QUERY )
else:
